...
i_url= 'https://occn-jira.oraclecorp.com/sr/jira.issueviews:searchrequest-csv-all-fields/'+filter+'/SearchRequest-'+filter+'.csv'
...
jsonurl='https://occn-jira.oraclecorp.com/rest/api/2/search?jql=project+%3D+EAS+AND+status+in+%28Open%2C+%22In+Progress%22%2C+%22L1+Approved%22%2C+%22L2+Approved%22%2C+%22L3+Approved%22%2C+Submitted%29+AND+component+%3D+SCPBI+AND+%22Object%22+in+%28EMDSAP%2C+EMDSAPDR%29+AND+%22Scheduled+Start+Time%22+%3E%3D+%220%22+AND+%22Scheduled+Start+Time%22+%3C%3D+3w+ORDER+BY+%22Scheduled+Start+Time%22%2C+%22Object%22+ASC'
j=s.get(jsonurl)
#print r
# dumping the HTML report page to CSV file
output='jira_'+filter+'.json'
f = open(output,"wb")
f.write(j.content)
f.close()

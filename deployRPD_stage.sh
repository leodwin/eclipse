...
echo  |tee -a ${WORKDIR}/${DEPLOYLOG}
echo "Step IV -  Update Target RPD Connection Pool" |tee -a ${WORKDIR}/${DEPLOYLOG}
$BINHOME/datamodel.sh updateConnectionpool -C ${BKPLOC}/${TENV}_con_pool.json -SI ssi -U ${WLSID} -P ${TWLSPW} |tee -a ${WORKDIR}/${DEPLOYLOG}
grep "Step IV -  Update Target RPD Connection Pool" -A 4 ${WORKDIR}/${DEPLOYLOG}|grep "Connection pool update completed successfully" >/dev/null
if [ $? -ne 0 ]
